...
- **本机历史与操作记录**：当你开启资源历史时，资源趋势、持续问题、应用关注规则的触发/恢复事件和你确认的操作结果会保存在设备上。应用和启动项名称只有在你允许保存名称后才会跨重启保留。连接历史使用单独开关且默认关闭；启用后保存应用名称、反向解析得到的主机名或远端 IP、协议、端口和五分钟观察次数，最多 5000 条，可保留 1、7 或 30 天。
- **应用清单缓存**：macOS 上会按界面语言在应用数据目录中保存已安装应用的本地化名称、路径、Bundle ID、应用大小、时间信息和应用目录指纹，减少重复扫描。缓存超过 24 小时会建议刷新，最多使用 7 天；手动刷新会重新读取，清单不会上传。
- **空间扫描结果**：常规扫描读取名称、大小、时间、位置和文件类型等元数据，结果最多保留 7 天。重复文件检查只在前台主动执行，读取候选内容计算哈希，结果当前不跨重启保存。清理会用保存的扫描证据逐项绑定已确认目标且不跟随符号链接；不存在的目标会跳过，无法访问或不支持的目标会单独报告，不阻塞其他项目。
...
- 打开网络质量页面会立即检查一次，并在页面保持打开时每 30 秒检查一次；离开页面后停止，也可以手动立即复测。每次检查会分别确认本地路由、系统 DNS 对 `example.com` 的解析、IPv4/IPv6、直连公网和独立的 `example.com:443` 服务，并用轻量 TCP 连接估算延迟、抖动和连接失败比例；不发送 HTTP 请求内容。
...
- “设置 → 关于与支持 → 清除全部本机数据”会清除 CoreRobin 的偏好、历史、提醒记录、应用清单缓存、文件洞察缓存和保存的空间扫描结果。也可以只在“记录”页清空历史，或只在清理页清除扫描结果。
