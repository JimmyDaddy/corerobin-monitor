...
- Long-term network-quality history is off by default. When enabled, a lightweight check runs every five minutes while the page is closed and stores five-minute aggregate buckets locally. You can select a one-hour or 24-hour window and clear it separately. Stored fields are limited to time, status, aggregate DNS/latency/jitter values, and successful/total TCP probe counts.
